...
//    mAMEnv.sustainPoint(1);
    ...
    createInternalTriggerParameter("releaseTime", 1.7, 0.1, 10.0);
    createInternalTriggerParameter("sustain", 0.75, 0.1, 1.0);
    createInternalTriggerParameter("pan", -1.0, -1.0, 1.0);
    createInternalTriggerParameter("amFunc", 0.0, 0.0, 3.0);
    createInternalTriggerParameter("am1", 0.1, 0.1, 1.0);
    createInternalTriggerParameter("am2", 0.1, 0.1, 1.0);
    createInternalTriggerParameter("amRise", 1.0, 0.1, 1.0);
    createInternalTriggerParameter("amRatio", 2.0, 0.1, 2.0);
  }
  ...
  virtual void onProcess(AudioIOData& io) override {
    ...
    while(io()){
      ...
      float s2 = mOsc() * mAmpEnv() * getInternalParameterValue("amplitude");
      float s3;
      mEnvFollow(s1);
      mPan(s1, s2, s3);
      io.out(0) += s1;
